-- ================================================
-- B2B Analytics Views for Aggregated Insights
-- ================================================

-- View: Top schools by engagement (views + list interactions)
CREATE OR REPLACE VIEW public.analytics_top_schools AS
SELECT 
  s.id AS school_id,
...
-- View: Top grades by volume (list views)
CREATE OR REPLACE VIEW public.analytics_top_grades AS
SELECT 
  g.id AS grade_id,
...
-- View: Top items by click (most clicked items across all stores)
CREATE OR REPLACE VIEW public.analytics_top_items AS
SELECT 
  mi.id AS item_id,
...
-- View: Conversion by store (cart views → clicks)
CREATE OR REPLACE VIEW public.analytics_store_conversion AS
SELECT 
  ps.id AS store_id,
...
-- View: Demand by region (aggregated by state/city)
CREATE OR REPLACE VIEW public.analytics_demand_by_region AS
SELECT 
  s.state,
  s.city,
...
-- View: Upload funnel metrics
CREATE OR REPLACE VIEW public.analytics_upload_funnel AS
SELECT 
...
-- ================================================
-- RLS Policies for Views (views inherit table policies)
-- Grant SELECT access to admins for analytics views
-- ================================================

-- Note: Views automatically use RLS from underlying tables.
-- Admins already have SELECT access to all event tables.

COMMENT ON VIEW public.analytics_top_schools IS 'Top schools by engagement score - for B2B dashboards';
COMMENT ON VIEW public.analytics_top_grades IS 'Top grades by list views - for understanding demand';
COMMENT ON VIEW public.analytics_top_items IS 'Most clicked items - for retailer insights';
COMMENT ON VIEW public.analytics_store_conversion IS 'Store click metrics - for partner reports';
COMMENT ON VIEW public.analytics_demand_by_region IS 'Regional demand aggregation - for market analysis';
COMMENT ON VIEW public.analytics_upload_funnel IS 'Upload funnel by event type - for product metrics';
COMMENT ON VIEW public.analytics_daily_summary IS 'Daily engagement summary - for trend analysis';