...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ========================================
    // ACTION 1: CREATE JOB - Just create the job entry
    // File will be uploaded directly to storage by the client
    // ========================================
    if (contentType.includes('application/json')) {
      const body = await req.json();
      
      // ========================================
      // ACTION: CREATE - Create import job (file already in storage)
      // ========================================
      if (body.action === 'create') {
        const { file_name, total_rows, file_path } = body;

        if (!file_name || !total_rows || !file_path) {
          return new Response(
            JSON.stringify({ error: 'file_name, total_rows e file_path são obrigatórios' }),
            { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
          );
        }

        console.log(`[create] Creating job for file: ${file_name}, rows: ${total_rows}`);
        ...
        console.log(`[create] Created job ${job.id}`);
      ...
      // Reads file from storage in chunks using range requests
      // ========================================
      if (body.action === 'process' && body.job_id) {
        ...
        // Use lines data passed from client if available (avoids re-downloading)
        let batchLines: string[] = [];
        let headerLine = '';
        
        if (body.lines && Array.isArray(body.lines)) {
          // Lines provided by client - header is first, rest are data
          headerLine = body.header_line || '';
          batchLines = body.lines;
        } else {
          // Fallback: Download CSV from storage (for resuming interrupted jobs)
          ...
          // Get cursor position and slice lines
          ...
          batchLines = allLines.slice(startLine, endLine);
        }
        ...
        console.log(`[process] Job ${jobId}: Processing ${batchLines.length} lines`);
        ...
        const newCursorLine = (job.cursor_line || 0) + batchLines.length;
        ...
        // Check if done
        const isDone = newProcessedRecords >= job.total_records;
      ...
      return new Response(
        JSON.stringify({ error: 'Ação inválida. Use: create, status, ou process' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
